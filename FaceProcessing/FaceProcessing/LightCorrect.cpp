...
void process(IplImage* sourImg)
{
	...
	orig = cvCreateImage(cvSize(sourImg->width, sourImg->height), 8, 3);
	cvCopy(sourImg, orig);

	cvSmooth(orig, orig, CV_GAUSSIAN, 3, 3);//add by Chen lee
	cvSmooth(orig, orig, CV_GAUSSIAN, 3, 3);//add by Chen lee
...
void LightCorrect(IplImage* sourImg)
{
	process(sourImg);
}

				...
				const CvMat* t, const AAM_PAW& paw, const AAM_PAW& refpaw)
{
	...
	const std::vector<std::vector<int> >& tri = paw.__tri;/*triangle vertexes index*/
	const std::vector<std::vector<int> >& rect1 = paw.__rect;// Is point(j,i) in boundary: not(-1), yes(index of pixel)
	const std::vector<std::vector<int> >& rect2 = refpaw.__rect;
	const std::vector<int>& pixTri = paw.__pixTri;//triangle index
	...
	for(int y = miny; y < maxy; y++)
	{
		...
		for(int x = minx; x < maxx; x++)
		{
			x1 = x-minx;
			idx1 = rect1[y1][x1];
			if(idx1 >= 0)//point(y1,x1) in boundary
			{
				tri_idx = pixTri[idx1];
				v1 = tri[tri_idx][0];
				v2 = tri[tri_idx][1];
...
//if the shape in the range of width and height
...
{
	int total = 0;
	__referenceWidth = refWidth;//default value is 100
	...
	for(int i = 0; i < pts_files.size(); i++)
	{
		...
		CvPoint2D32f  lt = DetShape[0][0], rb = DetShape[0][1];//get the left top and right bottom point of face area
		double x = (lt.x+rb.x)/2., y = (lt.y+rb.y)/2.;//get the center of face area
		double w = (-lt.x+rb.x), h = (-lt.y+rb.y);//get the width and height of face area
		
		Shape.Translate(-x, -y);//set the origin to be the center of face area
		Shape.ScaleXY(__referenceWidth/w, __referenceWidth/h);// Scale horizontal and vertical respectively

		__VJDetectShape += Shape;
		total ++;
	}
	__VJDetectShape /= total;//get the mean shape for the first time
}

//init shape from detected face area rect
bool AAM_Pyramid::InitShapeFromDetBox(AAM_Shape& Shape, VJfacedetect& facedetect, const IplImage* image)
{
	...
	if(!flag)	return false;//if no face, just skip
	
	CvPoint2D32f  lt = DetShape[0][0], rb = DetShape[0][1];//left top and right bottom point of face area
	Shape = __VJDetectShape;
	Shape.ScaleXY((-lt.x+rb.x)/__referenceWidth, (-lt.y+rb.y)/__referenceWidth);//scale to the size of 100*100
						...
						int level /* = 1 */)
{
	...
	for(int i = 0; i < level; i++)//level for the use of pyramid 
	{
		...
		__model[i]->Build(pts_files, img_files, 1.0/pow(2.0, i));//training for aam model
	}
}

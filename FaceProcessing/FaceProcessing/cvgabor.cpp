...
void CvGabor::conv_img(IplImage *src, IplImage *dst, int Type)
{
  ...
  case CV_GABOR_PHASE:
	 ...
	 for (int i=0; i<mat->rows; i++)
	 {
		 ...
		 {
			 float rData = cvGetReal2D(rmat, i, j);
			 float iData = cvGetReal2D(imat, i, j);	
			 if (abs(rData) < 1.0e-8)//in case of rData equals to 0
			 {
				 if (rData < 0)
				 {
					 rData = -0.000001;
			 ...
			 float val = atan(abs(iData/rData));
			 if ((rData > 0) && (iData > 0))//first quadrant
			 {
				
			 }
			 else if ((rData < 0) && (iData > 0))//second quadrant
			 {
				 val += 2/PI;
			 }
			 else if ((rData < 0) && (iData < 0))
			 {
				 val += PI;
			 }
			 else

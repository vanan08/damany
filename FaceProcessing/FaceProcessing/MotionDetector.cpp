...
//标识是否画出红色的框框,通过UI来设定
void FaceProcessing::CMotionDetector::SetDrawRect(bool draw)
{
	drawRect = draw;
}

//设置布控的警戒区域，通过UI来调用该函数;
void FaceProcessing::CMotionDetector::SetAlarmArea(const int leftX, const int leftY, const int rightX, const int rightY, bool draw)
{
	xLeftAlarm = leftX;
	yTopAlarm = leftY;
	xRightAlarm = rightX; 
...
void FaceProcessing::CMotionDetector::SetRectThr(const int fCount, const int gCount)
{
	faceCount = fCount;
	groupCount = gCount;
}
...
void FaceProcessing::CMotionDetector::FindRectX(IplImage *img, const int leftY, const int rightY)
{
...
void FaceProcessing::CMotionDetector::FindRectY(IplImage *img, const int leftX, const int rightX)
{
...
/*每次从摄像头获得一张图片后调用，进行运动目标检测，并画框，
  ...
  该函数由UI来调用*/
bool FaceProcessing::CMotionDetector::PreProcessFrame(Frame frame, Frame &lastFrame)
{
	Frame tempFrame; 
	tempFrame = prevFrame; 

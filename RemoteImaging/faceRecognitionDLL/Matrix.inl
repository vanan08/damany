...
{
	long double normResult=0.0;
	if(mIn.GetColNum()>1 && mIn.GetRowNum()==1){
		for(int i=0;i<mIn.GetColNum();i++){
			normResult += mIn(0,i) * mIn(0,i);
		}
		normResult = sqrt(normResult);
	}
	else if(mIn.GetColNum()==1 && mIn.GetRowNum()>1){
		for(int i=0;i<mIn.GetRowNum();i++){
			normResult += mIn(i,0) * mIn(i,0);
		}
		normResult = sqrt(normResult);
	}
...
template <class _Ty>
bool MatrixMean(matrix<_Ty>& mIn, matrix<_Ty>& mOut,int Dim=1)
{
	Assert(Dim>=1 && Dim<=2);
	if(1==Dim){	//mean through col, the mean result is a 1*N vector
		if(mOut.GetColNum()<mIn.GetColNum()) return 0;
		
		int inRowNum = mIn.GetRowNum();
		for(int col=0;col<mIn.GetColNum();col++){
			long double mean = 0.0;
			for(int row=0;row<mIn.GetRowNum();row++){
				mean += mIn(row, col);
			}
			mOut(0,col)=mean/inRowNum; // result stored in the 1st row of mOut
		}
	}
	else if(2==Dim){	//mean through row, the mean result is a N*1 vector
		if(mOut.GetRowNum()<mIn.GetRowNum()) return 0;
		
		int inColNum = mIn.GetColNum();
		for(int row=0;row<mIn.GetRowNum();row++){
			long double mean = 0.0;
			for(int col=0;col<mIn.GetColNum();col++){
				mean += mIn(row, col);
			}
			mOut(row,0)=mean/inColNum; // result stored in the 1st col of mOut
		}
	}
	return 1;
}
